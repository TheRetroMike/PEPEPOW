...
const char * const BITCOIN_CONF_FILENAME = "PEPEPOW.conf";
const char * const BITCOIN_PID_FILENAME = "PEPEPOWd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "PEPEPOW" is a composite category enabling all PEPEPOW-related debug output
            if(ptrCategory->count(string("PEPEPOW"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PEPEPOWCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PEPEPOWCore
    // Mac: ~/Library/Application Support/PEPEPOWCore
    // Unix: ~/.PEPEPOWcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PEPEPOWCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PEPEPOWCore";
#else
    // Unix
    return pathRet / ".PEPEPOWcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty PEPEPOW.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override PEPEPOW.conf
